...
<div id="map"></div>
    ...
    <script>
        ...
        var map = L.map('map').setView([9.7489, -83.7534], 8); // Initial center and zoom level for Costa Rica
        ...
        var customIcon = L.icon({
            iconUrl: '<? echo get_template_directory_uri() . '/images/marker.png'?>',
        ...
        for (var country in countryCoordinates) {
            var marker = L.marker(countryCoordinates[country], {icon: customIcon}).addTo(map);
        ...
        var countryProviders = {
            'costarica': [
                { name: 'Provider A', location: [9.7489, -83.7534], info: 'Provider A Info' },
                { name: 'Provider B', location: [10.3157, -84.8227], info: 'Provider B Info' }
                // Add more providers for Costa Rica
            ],
            'nicaragua': [
                { name: 'Provider 1', location: [12.8654, -85.2072], info: 'Provider 1 Info' },
                { name: 'Provider 2', location: [11.8251, -85.9083], info: 'Provider 2 Info' }
                // Add more providers for Nicaragua
            ],
            'panama': [
                { name: 'Provider X', location: [8.9824, -79.5199], info: 'Provider X Info' },
                { name: 'Provider Y', location: [9.1018, -79.4029], info: 'Provider Y Info' }
        ...
        function addMarkers(countryCode) {
            ...
            if (providers) {
                providers.forEach(function(provider) {
                    var marker = L.marker(provider.location, {icon: customIcon}).addTo(map);
